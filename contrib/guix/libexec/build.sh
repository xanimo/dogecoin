...
# CONFIGFLAGS
# Faithful to Dogecoin's gitian descriptors (contrib/gitian-descriptors/*.yml):
#   all: --enable-reduce-exports --disable-bench --disable-gui-tests
#   linux only: --enable-glibc-back-compat (glibc_compat wrapper caps symbol
#               versions to the symbol-check floors: 2.11 x86/arm, 2.17 aarch64)
# Dogecoin 1.14 has no --disable-fuzz-binary and no --disable-threadlocal.
