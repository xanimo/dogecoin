...
class AcceptBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        block_1443.solve()
        block_1442 = create_block(block_1443.sha256, create_coinbase(1444), block_1443.nTime+1)
        block_1442.solve()
        ...
        headers_message.headers.append(CBlockHeader(block_1442))
        test_node.send_message(headers_message)

        test_node.sync_with_ping()
        tip_entry_found = False
        for x in self.nodes[0].getchaintips():
            if x['hash'] == block_1442.hash:
                assert_equal(x['status'], "headers-only")
                tip_entry_found = True
        assert(tip_entry_found)
        assert_raises_jsonrpc(-1, "Block not found on disk", self.nodes[0].getblock, block_1442.hash)
        ...
        block_1445 = create_block(block_1442.sha256, create_coinbase(1445), block_1442.nTime+1)
        block_1445.solve()
        headers_message = msg_headers()
