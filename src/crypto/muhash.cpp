...
/** Count trailing zero bits in x. x must not be zero. */
...
inline limb_t ComputeDivstepMatrix(signed_limb_t eta, limb_t f, limb_t g, SignedMatrix& out)
{
    ...
    while (true) {
         ...
         * bits of g. No more than i can be cancelled out (as we'd be done before
         * that point), and no more than eta+1 can be done as its sign will flip
         * once that happens. */
...
Num3072 MuHash3072::ToNum3072(Span<const unsigned char> in) {
    ...
    // Hash the input with SHA256 to produce a 32-byte key
    unsigned char key[32];
    CSHA256().Write(in.data(), in.size()).Finalize(key);

    // Expand the 32-byte key to 384 bytes using ChaCha20
    // 384 = 6 * 64 (ChaCha20 block size), so this is exact.
    ChaCha20(key, 32).Output(tmp, sizeof(tmp));
    Num3072 out_num{tmp};
...
void MuHash3072::Finalize(uint256& out) noexcept
{
    ...
    CSHA256().Write(data, sizeof(data)).Finalize(out.begin());
}
