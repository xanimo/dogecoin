...
/** Perform a number of SHA-512 transformation, processing 128-byte chunks. (AVX2) */
void Transform_AVX2(uint64_t* s, const unsigned char* chunk, size_t blocks)
{
#ifdef USE_AVX2
...
/** Perform a number of SHA-512 transformation, processing 128-byte chunks. */
void Transform(uint64_t* s, const unsigned char* chunk, size_t blocks)
{
    while (blocks--) {
...
void (*transform_ptr) (uint64_t*, const unsigned char*, size_t) = &Transform;
...
CSHA512& CSHA512::Write(const unsigned char* data, size_t len)
{
    ...
    if (bufsize && bufsize + len >= 128) {
        ...
        bytes += 128 - bufsize;
        data += 128 - bufsize;
        sha512::transform_ptr(s, buf, 1);
        bufsize = 0;
    }
    if (end - data >= 128) {
        // Process full chunks directly from the source.
        size_t blocks = (end - data) / 128;
        sha512::transform_ptr(s, data, blocks);
        data += 128 * blocks;
        bytes += 128 * blocks;
    }
    if (end > data) {
