    ...
    : m_filter_type(filter_type)
{
    ...
    m_db = std::unique_ptr<BaseIndex::DB>(new BaseIndex::DB(path / "db", n_cache_size, f_memory, f_wipe));
    m_filter_fileseq = std::unique_ptr<FlatFileSeq>(new FlatFileSeq(std::move(path), "fltr", FLTR_FILE_CHUNK_SIZE));
}
...
bool BlockFilterIndex::CommitInternal(CDBBatch& batch)
{
    ...
    // Dogecoin: FileCommit returns void, so we can't check for failure.
    FileCommit(file.Get());
...
size_t BlockFilterIndex::WriteFilterToDisk(FlatFilePos& pos, const BlockFilter& filter)
{
    ...
    if (pos.nPos + data_size > MAX_FLTR_FILE_SIZE) {
        ...
        // Dogecoin: FileCommit returns void.
...
bool BlockFilterIndex::WriteBlock(const CBlock& block, const CBlockIndex* pindex)
{
    ...
    if (pindex->nHeight > 0) {
        // Dogecoin: UndoReadFromDisk takes (blockundo, pos, hashBlock) instead of (blockundo, pindex)
