...
bool AppInitParameterInteraction()
{
    ...
    hashAssumeValid = uint256S(gArgs.GetArg("-assumevalid", chainparams.GetConsensus(0).defaultAssumeValid.GetHex()));
    ...
    if (gArgs.IsArgSet("-maxtxfee"))
    {
        ...
        if (CFeeRate(::maxTxFee, 1000) < ::minRelayTxFeeRate)
        {
            return InitError(strprintf(_("Invalid amount for -maxtxfee=<amount>: '%s' (must be at least the minrelay fee of %s to prevent stuck transactions)"),
                                       gArgs.GetArg("-maxtxfee", ""), ::minRelayTxFeeRate.ToString()));
        }
    }
    ...
    if (gArgs.IsArgSet("-harddustlimit"))
    {
        CAmount n = nHardDustLimit;
        if (!ParseMoney(gArgs.GetArg("-harddustlimit", ""), n))
            return InitError(AmountErrMsg("harddustlimit", gArgs.GetArg("-harddustlimit", "")));
        nHardDustLimit = n;
    }

    if (gArgs.IsArgSet("-dustlimit"))
    {
        CAmount n = nDustLimit;
        if (!ParseMoney(gArgs.GetArg("-dustlimit", ""), n))
            return InitError(AmountErrMsg("dustlimit", gArgs.GetArg("-dustlimit", "")));
    ...
    fUseNamecoinApi = gArgs.GetBoolArg("-rpcnamecoinapi", DEFAULT_USE_NAMECOIN_API);
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (proxyArg != "" && proxyArg != "0") {
        ...
        SetReachable(NET_TOR, true); // by default, -proxy sets onion as reachable, unless -noonion later
    }
