...
{
    return MallocUsage(sizeof(unordered_node<X>)) * m.size() + MallocUsage(sizeof(void*) * m.bucket_count());
}

template<typename X, typename Y>
static inline size_t DynamicUsage(const std::unordered_set<X, Y>& s)
{
    return MallocUsage(sizeof(unordered_node<std::pair<const X, Y> >)) * s.size() + MallocUsage(sizeof(void*) * s.bucket_count());
}
