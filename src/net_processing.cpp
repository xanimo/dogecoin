...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (!ProcessNewBlockHeaders(headers, state, chainparams, &pindexLast)) {
            if (state.IsInvalid()) {
                ...
                // Disconnect peers on chains incompatible with ours.
                // (Adapted from Bitcoin Core 37886d5e2f: "Disconnect outbound
                // peers relaying invalid headers".  Extended here to also
                // disconnect inbound peers: a peer advertising headers on a
                // known-invalid chain should be disconnected regardless of
                // direction, since it is either adversarial or severely out of
                // sync.  We exempt whitelisted/addnode peers to preserve
                // operational flexibility.)
                if (!pfrom->fWhitelisted && !pfrom->fAddnode) {
                    pfrom->fDisconnect = true;
                }
                return error("invalid header received");
            }
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        bool forceProcessing = pfrom->fWhitelisted && !ChainstateActive().IsInitialBlockDownload();
...
bool SendMessages(CNode* pto, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    const Consensus::Params& consensusParams = Params().GetConsensus(chainActive.Height());
    {
        ...
        TRY_LOCK(cs_main, lockMain); // Acquire cs_main for ChainstateActive().IsInitialBlockDownload() and CNodeState()
        if (!lockMain)
            return true;
        ...
        if (!ChainstateActive().IsInitialBlockDownload() && pto->nNextLocalAddrSend < current_time) {
            AdvertiseLocal(pto);
        ...
        //
        std::vector<CInv> vGetData;
        if (!pto->fClient && (fFetch || !ChainstateActive().IsInitialBlockDownload()) && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
