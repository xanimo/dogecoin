...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::VERSION)
    {
        ...
        pfrom->SetRecvVersion(greatest_common_version);
        pfrom->nVersion = nVersion;

        const CNetMsgMaker msg_maker(greatest_common_version);

        if (greatest_common_version >= WTXID_RELAY_VERSION) {
            connman.PushMessage(pfrom, msg_maker.Make(NetMsgType::TX));
        }
        ...
        // pfrom->fClient = (!(nServices & NODE_NETWORK) && !(nServices & "NETWORK_LIMITED"));

        // // set nodes not capable of serving the complete blockchain history as "limited nodes"
        // pfrom->m_limited_node = (!(nServices & NODE_NETWORK) && (nServices & "NETWORK_LIMITED"));
    ...
    if (strCommand == NetMsgType::ADDR || strCommand == NetMsgType::ADDRV2) {
        ...
        if (vAddr.size() > 1000)
        {
            ...
            strprintf("%s message size = %u", strCommand, vAddr.size());
        }
