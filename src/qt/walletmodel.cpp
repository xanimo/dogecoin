...
{
    LOCK(wallet->cs_wallet);
    for (const std::pair<std::CTxDestination, CAddressBookData>& item : wallet->mapAddressBook)
        for (const std::pair<std::std::string, std::string>& item2 : item.second.destdata)
