...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
    ...
    bool solved = SignStep(provider, creator, fromPubKey, result, whichType, SIGVERSION_BASE, sigdata);
    bool P2SH = false;
    CScript subscript;
    ...
    if (solved && whichType == TX_SCRIPTHASH)
    {
        ...
        solved = solved && SignStep(provider, creator, subscript, result, whichType, SIGVERSION_BASE, sigdata) && whichType != TX_SCRIPTHASH;
        P2SH = true;
    }
    ...
    if (solved && whichType == TX_WITNESS_V0_KEYHASH)
    {
        ...
        txnouttype subType;
        solved = solved && SignStep(provider, creator, witnessscript, result, subType, SIGVERSION_WITNESS_V0, sigdata);
    ...
    else if (solved && whichType == TX_WITNESS_V0_SCRIPTHASH)
    {
        ...
        txnouttype subType;
        solved = solved && SignStep(provider, creator, witnessscript, result, subType, SIGVERSION_WITNESS_V0, sigdata) && subType != TX_SCRIPTHASH && subType != TX_WITNESS_V0_SCRIPTHASH && subType != TX_WITNESS_V0_KEYHASH;
        result.push_back(std::vector<unsigned char>(witnessscript.begin(), witnessscript.end()));
        sigdata.scriptWitness.stack = result;
        sigdata.witness = true;
        result.clear();
    } else if (solved && false /* TX_WITNESS_UNKNOWN N/A */) {
        sigdata.witness = true;
    }
