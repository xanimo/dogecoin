...
BOOST_AUTO_TEST_CASE(caddrinfo_get_tried_bucket)
{
    ...
    //  this test could be a security issue.
    BOOST_CHECK(info1.GetTriedBucket(nKey1, asmap1) != info1.GetTriedBucket(nKey2, asmap1));
    ...
    BOOST_CHECK(info1.GetKey() != info2.GetKey());
    BOOST_CHECK(info1.GetTriedBucket(nKey1, asmap1) != info2.GetTriedBucket(nKey1, asmap1));
    ...
    for (int j = 0; j < 255; j++) {
        ...
        int bucket = infoj.GetTriedBucket(nKey1, asmap1);
        buckets.insert(bucket);
    }
...
BOOST_AUTO_TEST_CASE(caddrinfo_get_new_bucket)
{
    ...
    BOOST_CHECK_EQUAL(info1.GetNewBucket(nKey1, source1, asmap1), 795);

    // Test 30: Make sure key actually randomizes bucket placement. A fail on
    //  this test could be a security issue.
    BOOST_CHECK(info1.GetNewBucket(nKey1, asmap1) != info1.GetNewBucket(nKey2, asmap1));

    // Test 31: Ports should not effect bucket placement in the addr
    CAddrInfo info2 = CAddrInfo(addr2, source1);
    BOOST_CHECK(info1.GetKey() != info2.GetKey());
    BOOST_CHECK_EQUAL(info1.GetNewBucket(nKey1, asmap1), info2.GetNewBucket(nKey1, asmap1));
    ...
    for (int i = 0; i < 255; i++) {
        CAddrInfo infoi = CAddrInfo(
            CAddress(ResolveService("250.1.1." + std::to_string(i)), NODE_NONE),
            ResolveIP("250.1.1." + std::to_string(i)));
        int bucket = infoi.GetNewBucket(nKey1, asmap1);
        buckets.insert(bucket);
    }
    ...
    for (int j = 0; j < 4 * 255; j++) {
        CAddrInfo infoj = CAddrInfo(CAddress(
            ...
            ResolveIP("251.4.1.1"));
        int bucket = infoj.GetNewBucket(nKey1, asmap1);
        buckets.insert(bucket);
    }
