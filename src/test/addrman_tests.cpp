...
BOOST_AUTO_TEST_CASE(caddrinfo_get_tried_bucket)
{
    ...
    std::vector<bool> asmap_raw = FromBytes(asmap, sizeof(asmap) * 8);

    BOOST_CHECK_EQUAL(info1.GetTriedBucket(nKey1, asmap_raw), 236);

    // Test: Make sure key actually randomizes bucket placement. A fail on
    //  this test could be a security issue.
    BOOST_CHECK(info1.GetTriedBucket(nKey1, asmap_raw) != info1.GetTriedBucket(nKey2, asmap_raw));
    ...
    BOOST_CHECK(info1.GetKey() != info2.GetKey());
    BOOST_CHECK(info1.GetTriedBucket(nKey1, asmap_raw) != info2.GetTriedBucket(nKey1, asmap_raw));
    ...
    for (int j = 0; j < 255; j++) {
        ...
        int bucket = infoj.GetTriedBucket(nKey1, asmap_raw);
        buckets.insert(bucket);
    }
...
BOOST_AUTO_TEST_CASE(caddrinfo_get_new_bucket)
{
    ...
    // Test: Make sure the buckets are what we expect
    BOOST_CHECK_EQUAL(info1.GetNewBucket(nKey1, asmap_raw), 795);
    BOOST_CHECK_EQUAL(info1.GetNewBucket(nKey1, source1, asmap_raw), 795);

    // Test: Make sure key actually randomizes bucket placement. A fail on
    //  this test could be a security issue.
    BOOST_CHECK(info1.GetNewBucket(nKey1, asmap_raw) != info1.GetNewBucket(nKey2, asmap_raw));

    // Test: Ports should not affect bucket placement in the addr
    CAddrInfo info2 = CAddrInfo(addr2, source1);
    BOOST_CHECK(info1.GetKey() != info2.GetKey());
    BOOST_CHECK_EQUAL(info1.GetNewBucket(nKey1, asmap_raw), info2.GetNewBucket(nKey1, asmap_raw));
    ...
    for (int i = 0; i < 255; i++) {
        CAddrInfo infoi = CAddrInfo(
            CAddress(ResolveService("250.1.1." + std::to_string(i)), NODE_NONE),
            ResolveIP("250.1.1." + std::to_string(i)));
        int bucket = infoi.GetNewBucket(nKey1, asmap_raw);
        buckets.insert(bucket);
    }
    ...
    for (int j = 0; j < 4 * 255; j++) {
        CAddrInfo infoj = CAddrInfo(CAddress(
                                        ResolveService(
                                            std::to_string(250 + (j / 255)) + "." + std::to_string(j % 256) + ".1.1"), NODE_NONE),
            ResolveIP("251.4.1.1"));
        int bucket = infoj.GetNewBucket(nKey1, asmap_raw);
        buckets.insert(bucket);
    }
...
BOOST_AUTO_TEST_CASE(addrman_serialization)
{
    ...
    CAddrManTest addrman_asmap1(true, asmap_raw);
    CAddrManTest addrman_asmap1_dup(true, asmap_raw);
    ...
    //BOOST_CHECK(bucketAndEntry_asmap1_deser_addr1.second != bucketAndEntry_asmap1_deser_addr2.second); FIXME
}
