    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        // Do a random modification.
        {
            ...
            if (InsecureRandRange(5) == 0 || coin.IsPruned()) {
                ...
                stack.back()->AddCoin(COutPoint(txid, 0), std::move(newcoin), !coin.IsPruned() || InsecureRand32() & 1);
            } else {
