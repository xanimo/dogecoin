...
static std::vector<CAddress> MakeFixtureAddresses()
{
    auto make_addr = [](const CService& s, ServiceFlags services, uint32_t nTime) {
        CAddress a(s, services);
        a.nTime = nTime;
        return a;
    };
...
static const std::vector<CAddress> fixture_addresses = MakeFixtureAddresses();
