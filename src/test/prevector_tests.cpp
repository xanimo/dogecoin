...
class prevector_tester {
    ...
    void test() {
        ...
        for (auto rit = pre_vector.rbegin(); rit != pre_vector.rend(); ++rit) {
             local_check(*rit == real_vector[--pos]);
        }
        for (const T& v : const_pre_vector) {
             local_check(v == real_vector[pos++]);
        }
        for (auto rit = const_pre_vector.rbegin(); rit != const_pre_vector.rend(); ++rit) {
             local_check(*rit == real_vector[--pos]);
        }
