...
BOOST_AUTO_TEST_CASE(varints)
{
    ...
    for (uint64_t i = 0;  i < 100000000000ULL; i += 999999937) {
        ...
        size += ::GetSerializeSize(VARINT(i, VarIntMode::DEFAULT), 0, 0);
        BOOST_CHECK(size == ss.size());
    }
    ...
    for (uint64_t i = 0;  i < 100000000000ULL; i += 999999937) {
        uint64_t j = -1;
        ss >> VARINT(j, VarIntMode::DEFAULT);
        BOOST_CHECK_MESSAGE(i == j, "decoded:" << j << " expected:" << i);
    }
}
...
BOOST_AUTO_TEST_CASE(varints_bitpatterns)
{
    ...
    ss << VARINT((uint8_t)0x80, VarIntMode::DEFAULT); BOOST_CHECK_EQUAL(HexStr(ss), "8000"); ss.clear();
    ...
    ss << VARINT((uint16_t)0xffff, VarIntMode::DEFAULT); BOOST_CHECK_EQUAL(HexStr(ss), "82fe7f"); ss.clear();
    ...
    ss << VARINT(0x80123456U, VarIntMode::DEFAULT); BOOST_CHECK_EQUAL(HexStr(ss), "86ffc7e756"); ss.clear();
    ss << VARINT((uint32_t)0x80123456U, VarIntMode::DEFAULT); BOOST_CHECK_EQUAL(HexStr(ss), "86ffc7e756"); ss.clear();
    ss << VARINT(0xffffffff, VarIntMode::DEFAULT); BOOST_CHECK_EQUAL(HexStr(ss), "8efefefe7f"); ss.clear();
    ss << VARINT(0x7fffffffffffffffLL, VarIntMode::NONNEGATIVE_SIGNED); BOOST_CHECK_EQUAL(HexStr(ss), "fefefefefefefefe7f"); ss.clear();
    ss << VARINT(0xffffffffffffffffULL, VarIntMode::DEFAULT); BOOST_CHECK_EQUAL(HexStr(ss), "80fefefefefefefefe7f"); ss.clear();
}
