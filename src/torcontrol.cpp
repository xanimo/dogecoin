    ...
    reconnect_timeout(RECONNECT_TIMEOUT_START)
{
    ...
    if (!conn.Connect(_target, [this](TorControlConnection& c){ connected_cb(c); },
         [this](TorControlConnection& c){ disconnected_cb(c); } )) {
...
void TorController::auth_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        _conn.Command(strprintf("ADD_ONION %s Port=%i,127.0.0.1:%i", private_key, GetListenPort(), GetListenPort()),
            [this](TorControlConnection& c, const TorControlReply& r){ add_onion_cb(c, r); });
    } else {
...
void TorController::authchallenge_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        if (l.first == "AUTHCHALLENGE") {
            ...
            _conn.Command("AUTHENTICATE " + HexStr(computedClientHash), [this](TorControlConnection& c, const TorControlReply& r){ auth_cb(c, r); });
        } else {
    ...
    if (reply.code == 250) {
        ...
        if (!torpassword.empty()) {
            if (methods.count("HASHEDPASSWORD")) {
                ...
                _conn.Command("AUTHENTICATE \"" + torpassword + "\"", [this](TorControlConnection& c, const TorControlReply& r){ auth_cb(c, r); });
            } else {
                LogPrintf("tor: Password provided with -torpassword, but HASHEDPASSWORD authentication is not available\n");
            }
        } else if (methods.count("NULL")) {
            LogPrint("tor", "tor: Using NULL authentication\n");
            _conn.Command("AUTHENTICATE", [this](TorControlConnection& c, const TorControlReply& r){ auth_cb(c, r); });
            ...
            if (status_cookie.first && status_cookie.second.size() == TOR_COOKIE_SIZE) {
                ...
                _conn.Command("AUTHCHALLENGE SAFECOOKIE " + HexStr(clientNonce), [this](TorControlConnection& c, const TorControlReply& r){ authchallenge_cb(c, r); });
            } else {
...
void TorController::connected_cb(TorControlConnection& _conn)
{
    ...
    if (!_conn.Command("PROTOCOLINFO 1", [this](TorControlConnection& c, const TorControlReply& r){ protocolinfo_cb(c, r); }))
...
void TorController::Reconnect()
{
     ...
     * may be restarting.
     */
    if (!conn.Connect(target, [this](TorControlConnection& c){ connected_cb(c); },
         [this](TorControlConnection& c){ disconnected_cb(c); } )) {
...
void StartTorControl(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    torControlThread = boost::thread([]{ TraceThread<void (*)()>("torcontrol", &TorControlThread); });
}
