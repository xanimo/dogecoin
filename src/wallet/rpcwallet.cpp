...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    BOOST_FOREACH(std::set<CTxDestination> grouping, pwallet->GetAddressGroupings())
    {
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Dogecoin address: ")+input.get_str());
            if (setAddress.count(address))
...
UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    const int64_t maxNewTxSize = CalculateMaximumSignedTxSize(*wtx.tx, *pwalletMain);
