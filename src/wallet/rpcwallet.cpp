...
UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    for (auto& input : tx.vin) {
        ...
        if (!ProduceSignature(*pwalletMain, MutableTransactionSignatureCreator(&tx, nIn, amount, SIGHASH_ALL), scriptPubKey, sigdata)) {
...
// PSBT support helpers (v0.14 adapter)

// Forward declarations for symbols defined in src/core_read.cpp.
// ParseSighashString is declared in core_io.h.

// Build a CMutableTransaction from inputs/outputs/locktime/rbf UniValues,
// mirroring the inlined logic in createrawtransaction / createpsbt.
static CMutableTransaction ConstructTransaction(const UniValue& inputs_in, const UniValue& outputs_in, const UniValue& locktime, const UniValue& rbf)
{
    if (inputs_in.isNull() || outputs_in.isNull())
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, inputs/outputs must be non-null");

    UniValue inputs = inputs_in.get_array();
    bool rbfOptIn = !rbf.isNull() && rbf.get_bool();
    ...
    for (unsigned int idx = 0; idx < inputs.size(); idx++) {
        ...
        if (nOutput < 0)
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, vout must be positive");
        uint32_t nSequence = rbfOptIn ? (std::numeric_limits<uint32_t>::max() - 2) : (rawTx.nLockTime ? std::numeric_limits<uint32_t>::max() - 1 : std::numeric_limits<uint32_t>::max());
        ...
        rawTx.vin.push_back(CTxIn(COutPoint(txid, nOutput), CScript(), nSequence));
    }

    std::set<CBitcoinAddress> setAddress;
    auto add_outputs = [&](const UniValue& obj){
        std::vector<std::string> addrList = obj.getKeys();
        for (const std::string& name_ : addrList) {
            if (name_ == "data") {
                std::vector<unsigned char> data = ParseHexV(obj[name_].getValStr(), "Data");
                rawTx.vout.push_back(CTxOut(0, CScript() << OP_RETURN << data));
            } else {
                CBitcoinAddress address(name_);
                if (!address.IsValid())
                ...
                setAddress.insert(address);
                CScript scriptPubKey = GetScriptForDestination(address.Get());
                CAmount nAmount = AmountFromValue(obj[name_]);
    ...
    if (outputs_in.isObject()) {
        add_outputs(outputs_in.get_obj());
    } else {
        UniValue outputs = outputs_in.get_array();
        for (unsigned int idx = 0; idx < outputs.size(); idx++) {
            add_outputs(outputs[idx].get_obj());
        }
    }
...
// Adapter for v0.14's CWallet::FundTransaction signature.
static void FundTransaction(CWallet* pwallet, CMutableTransaction& tx, CAmount& fee_out, int& change_position, UniValue options)
{
    CTxDestination changeAddress = CNoDestination();
    change_position = -1;
    bool includeWatching = false;
    ...
    if (!options.isNull()) {
        if (options.type() == UniValue::VBOOL) {
            includeWatching = options.get_bool();
        } else {
            ...
            if (options.exists("subtractFeeFromOutputs")) {
                UniValue subtractFeeFromOutputs = options["subtractFeeFromOutputs"].get_array();
                for (unsigned int idx = 0; idx < subtractFeeFromOutputs.size(); idx++) {
                    int pos = subtractFeeFromOutputs[idx].get_int();
                    if (pos < 0 || pos >= int(tx.vout.size()))
                        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, position out of range");
                    setSubtractFeeFromOutputs.insert(pos);
                }
            }
        }
    ...
    if (!pwallet->FundTransaction(tx, fee_out, overrideEstimatedFeerate, feeRate, change_position, strFailReason, includeWatching, lockUnspents, setSubtractFeeFromOutputs, reserveChangeKey, changeAddress))
        throw JSONRPCError(RPC_WALLET_ERROR, strFailReason);
}
...
UniValue walletcreatefundedpsbt(const JSONRPCRequest& request)
{
    ...
    if (!request.params[2].isNull()) RPCTypeCheckArgument(request.params[2], UniValue::VNUM);
    if (!request.params[3].isNull()) RPCTypeCheckArgument(request.params[3], UniValue::VBOOL);
    if (!request.params[4].isNull()) RPCTypeCheckArgument(request.params[4], UniValue::VOBJ);
...
{ //  category              name                        actor (function)           okSafeMode
    ...
    { "wallet",             "walletprocesspsbt",        &walletprocesspsbt,        true,   {"psbt","sign","sighashtype","bip32derivs"} },
    { "wallet",             "walletcreatefundedpsbt",   &walletcreatefundedpsbt,   true,   {"inputs","outputs","locktime","replaceable","options","bip32derivs"} },
